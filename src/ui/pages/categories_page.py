...
from src.services.excel_service import Category, ExcelService
from src.ui.components.new_category_dialog import NewCategoryDialog
from src.ui.pages.base_page import BasePage

LIST_COLUMNS = ("Category Code", "Category Name")


class CategoriesPage(BasePage):
    """Displays and manages categories from the Excel workbook."""
    ...
    def _build_toolbar(self) -> None:
        ...
        new_button = ctk.CTkButton(
            ...
            hover_color="#2ea043",
            command=self._open_new_category_dialog,
        )
    ...
    def refresh(self) -> None:
        """Reload categories from Excel and redraw the list."""
        ...
        if not categories:
            empty_label = ctk.CTkLabel(
                self._rows_frame,
                text="No categories yet. Click New Category to add one.",
                font=(Theme.FONT_FAMILY, Theme.FONT_SIZE_MD),
                text_color=Theme.TEXT_MUTED,
                anchor="w",
            )
            empty_label.grid(row=0, column=0, columnspan=2, padx=16, pady=24, sticky="w")
            return

        for row_index, category in enumerate(categories):
            self._add_row(row_index, category)

    def _add_row(self, row_index: int, category: Category) -> None:
        assert self._rows_frame is not None

        values = (category.category_code, category.category_name)
    ...
    def _open_new_category_dialog(self) -> None:
        NewCategoryDialog(self, on_save=self._save_category)

    def _save_category(self, category_code: str, category_name: str) -> None:
        ExcelService.add_category(category_code, category_name)
        self.refresh()
