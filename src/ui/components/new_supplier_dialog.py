...
class NewSupplierDialog(ctk.CTkToplevel):
    """Modal dialog for entering new supplier details."""
    ...
    def _build(self) -> None:
        ...
        self._name_entry = self._add_field(form, "Supplier Name")
        self._vat_entry = self._add_field(form, "VAT Number")
        self._contact_entry = self._add_field(form, "Contact Person")
        self._telephone_entry = self._add_field(form, "Telephone")
        self._email_entry = self._add_field(form, "Email")
    ...
    def _handle_save(self) -> None:
        supplier_name = self._name_entry.get().strip()

        if not supplier_name:
            self._error_label.configure(text="Supplier Name is required.")
            return

        self._on_save(
            supplier_name,
            self._vat_entry.get().strip(),
            self._contact_entry.get().strip(),
            self._telephone_entry.get().strip(),
            self._email_entry.get().strip(),
        )
        self._close()
